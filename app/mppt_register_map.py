...
# -*- coding: utf-8 -*-
# ------------------------------------------------------------------
# 📌 佛山金廣源 Ampinvt MPPT V1.1 - 寄存器映射表 (V4.2 修正識別模式)
# ------------------------------------------------------------------
...
B1_INFO = [
    ...
    },
    {
        ...
        # 🟢 [修正] 擴充 Map 定義，將 2~8 也視為手動模式 (對應電池串數)
        "map": { 
            0: "自動識別", 
            1: "手動設定", 
            2: "手動(24V)", 
    ...
    },
    {
        ...
        "ha": {"type": "sensor", "icon": "mdi:battery-plus"} # 1=12V, 2=24V, 4=48V
    },
    {
    ...
    {
        "key": "baud_rate", "name": "通訊波特率", "unit": None, "scale": 1, "offset": 13, "length": 1, "signed": False,
    ...
    # --- 電流參數 ---
    {"key": "hw_max_charge_current", "name": "硬體最大充電電流", "unit": "A", "scale": 100, "offset": 24, "length": 2, "signed": False, "ha": {"type": "sensor", "icon": "mdi:microchip"}},
...
D0_PARAMS = {
    ...
    0x09: {
        ...
        "ha": {
            "type": "select", "options": ["鉛酸(免維護)", "鉛酸(膠體)", "鉛酸(液體)", "鋰電池"], 
    ...
    0x0A: {
        ...
        "data_len": 1, "scale": 1, "valid_bytes": [6],
        "ha": {
            "type": "number", "min": 1, "max": 16, "step": 1, "mode": "box", 
            "icon": "mdi:battery-plus", "link_b1": "battery_count"
        }
    },
    # 0x0B: 識別方式
    0x0B: {
    ...
    0x0C: {
        ...
        "data_len": 1, "scale": 1, "valid_bytes": [6],
        "ha": {
            "type": "select", "options": ["關閉", "自動(光控+時控)", "時間控制", "光控模式", "遠程控制"],
    ...
    # --- 電壓與電流參數 ---
    0x21: {
        "key": "set_equalize_voltage", "name": "設定-均充電壓", "unit": "V", 
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 9.0, "max": 100.0, "step": 0.1, "mode": "box", "link_b1": "equalize_voltage"}
    },
    0x22: {
        "key": "set_float_voltage", "name": "設定-浮充電壓", "unit": "V", 
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 9.0, "max": 100.0, "step": 0.1, "mode": "box", "link_b1": "float_voltage"}
    },
    0x23: {
        "key": "set_discharge_limit", "name": "設定-放電下限電壓", "unit": "V",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 9.0, "max": 100.0, "step": 0.1, "mode": "box", "link_b1": "discharge_limit_voltage"}
    },
    0x25: {
        "key": "set_max_charge_curr", "name": "設定-最大充電電流", "unit": "A",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 0.0, "max": 60.0, "step": 1.0, "mode": "slider", "link_b1": "max_charge_current"}
    },
    0x26: {
        "key": "set_discharge_recover", "name": "設定-過放恢復電壓", "unit": "V",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 9.0, "max": 100.0, "step": 0.1, "mode": "box", "link_b1": "discharge_recovery_voltage"}
    },
    0x27: {
        "key": "set_over_prot_vol", "name": "設定-過壓保護電壓", "unit": "V",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 9.0, "max": 100.0, "step": 0.1, "mode": "box", "link_b1": "over_voltage_protection"}
    },
    0x28: {
        "key": "set_over_recover_vol", "name": "設定-過壓恢復電壓", "unit": "V",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 9.0, "max": 100.0, "step": 0.1, "mode": "box", "link_b1": "over_voltage_recovery"}
    },

    # --- 光控參數 ---
    0x29: {
        "key": "set_light_on_vol", "name": "設定-光控開啟電壓", "unit": "V",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 0.0, "max": 50.0, "step": 1.0, "mode": "box", "link_b1": "light_control_on_voltage"}
    },
    0x2A: {
        "key": "set_light_off_vol", "name": "設定-光控關閉電壓", "unit": "V",
        "data_len": 2, "scale": 0.01, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 0.0, "max": 50.0, "step": 1.0, "mode": "box", "link_b1": "light_control_off_voltage"}
    },
    0x2B: {
        "key": "set_light_on_delay", "name": "設定-光控開啟延遲", "unit": "s",
        "data_len": 2, "scale": 1, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 0, "max": 999, "step": 1, "mode": "box", "link_b1": "light_control_on_delay"}
    },
    0x2C: {
        "key": "set_light_off_delay", "name": "設定-光控關閉延遲", "unit": "s",
        "data_len": 2, "scale": 1, "valid_bytes": [5, 6],
        "ha": {"type": "number", "min": 0, "max": 999, "step": 1, "mode": "box", "link_b1": "light_control_off_delay"}
    },
}
