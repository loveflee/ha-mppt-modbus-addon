# =============================================================================
# bus_master.py - V3.7 工業封存版
# 相容：HAManager V2.7、RobustMQTTClient V1.8
# 修復：isinstance 空 dict regression、版本號修正、_record_failure log 補回
# =============================================================================
...
class BusMasterScheduler:
    ...
    decode 型別強制：攔截劣質 Adapter 的 None / list / str 回傳
    """
    ...
    def unregister_device(self, uid: int):
        """動態註銷；slow_heap 殘留條目由 _arbitration_loop 懶刪除"""
    ...
    async def submit_write(self, uid: int, key: str, value):
        """推入寫入任務，同 uid+key 自動覆蓋舊值（OOM 免疫）"""
        if uid not in self.adapters:
            return
    ...
    async def _get_next_write_task(self):
        """next(iter()) 相容普通 dict，不需要 OrderedDict"""
        async with self.write_lock:
    ...
    async def _arbitration_loop(self):
        """
        核心調度迴圈：完美掛起、heap 懶刪除、防餓死、全局例外防護
        """
        while self.running:
            try:
                # heap 懶刪除：彈出已 unregister 的設備（O(1) 攤銷，比 heapify 快）
    ...
    async def _process_write(self, task):
        ...
        physical_fault_count 計數器（非旗標）：
          Timeout / CRC / 未預期例外 → 累積
          邏輯拒絕 / 值不符 → 不累積
          3 次後 >= 2 → _record_failure，否則 _record_success
        """
        uid, key, value = task
        adapter = self.adapters.get(uid)
        ha_mgr = self.ha_managers.get(uid)
        if not adapter or not ha_mgr:
            return  # 已 unregister，自然消耗
        ...
        for attempt in range(1, 4):
            ...
            if raw_data:
                try:
                    ...
                    # 型別強制校驗：同時攔截 None 與空 dict {}
                    # None → isinstance 失敗；{} → not decoded 失敗
    ...
    def _record_failure(self, uid: int):
        ...
        logger.error(f"[{uid}] 通訊失敗，累計 {state['timeout_count']} 次")  # 無人值守監控必要
